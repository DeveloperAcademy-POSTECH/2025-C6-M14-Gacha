...
struct ProgressHistoryView: View {
    ...
    private var romAverage: Int {
        ...
        let sum = recentRecords.map { $0.ROM }.reduce(0, +)
        return Int(sum / Double(recentRecords.count))
    }
    
    private var painMin: Int {
        guard let min = recentRecords.map({ $0.painLevel }).min() else { return 0 }
        return Int(min)
    }
    
    private var painMax: Int {
        guard let max = recentRecords.map({ $0.painLevel }).max() else { return 0 }
        return Int(max)
    }
